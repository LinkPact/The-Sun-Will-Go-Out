    ...
    public class Cursor
    {
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            if (isVisible)
            {
                ...
                {
                    if (isActive)
                        spriteBatch.Draw(active.Texture, position, active.SourceRectangle, Color.White, 0, origin, 1.0f, SpriteEffects.None, 0.5f);
                    if (!isActive)
                        spriteBatch.Draw(passive.Texture, position, passive.SourceRectangle, Color.White, 0, origin, 1.0f, SpriteEffects.None, 0.5f);
                }
                else
                {
                    if (isActive)
                        spriteBatch.Draw(activeSmall.Texture, position, activeSmall.SourceRectangle, Color.White, 0, origin, 1.0f, SpriteEffects.None, 0.5f);
                    if (!isActive)
                        spriteBatch.Draw(passiveSmall.Texture, position, passiveSmall.SourceRectangle, Color.White, 0, origin, 1.0f, SpriteEffects.None, 0.5f);
                }
            }   
        }
    }
